...
class ModelFile:
    ...
    """
    def show(self):
        klen = self.keysuplen + 2
        vlen = self.valsuplen + 2
        klenhalf = klen/2 - 3
        vlenhalf = vlen/2 - 3
        str = "+" + klen * '-' + '+' + vlen * '-' + '+\n'
        str += "|" + klenhalf * ' ' + 'Param ' + klenhalf * ' '
        str += '|' + vlenhalf * ' ' + 'Value ' + vlenhalf * ' ' + '|\n'
        str += "+" + klen * '-' + '+' + vlen * '-' + '+\n'
        for k, v in self:
            str += "| %s%s | %s%s |\n" % (k, (self.keysuplen-len(k)) * ' ', v, (self.valsuplen-len(v)) * ' ')
        str += "+" + klen * '-' + '+' + vlen * '-' + '+\n'
        print str
    """
    ...
    def save(self, path):
        f = open(path, 'w+')
        f.write("""########################################################################
# Automatically generated by shine                                     #
########################################################################
""")
