...
class Target(Component, Disk):
    ...
    def set_state(self, value, node=None):
        """
        Update target state on the specified node (or current node otherwise).
        """
        if node is not None:
            self._states[node] = value
        else:
    ...
    def update(self, other):
        ...
        if other._states[srvname] in (MOUNTED, RECOVERING):
